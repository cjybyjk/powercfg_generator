# XiGuaNew(final) flashable
A script for flash XiGuaNew(final) into your device
...
-   下载zip到你的设备中 
	...
	- [下载最新版本(final)卡刷包](XiGuaNew(final).Installer.final.zip) 

		[(Download newest (final) zip)](XiGuaNew(final).Installer.final.zip)
...
### 移除
-	下载 [XiGuaNew(final) Remover](XiGuaNew(final).Remover.zip) 并刷入

	Download [XiGuaNew(final) Remover](XiGuaNew(final).Remover.zip) and flash it
